...
auto sum_values(const map_type& map) {
    ...
    return std::accumulate(values.begin(), values.end(), typename map_type::mapped_type{});
}
...
template<typename map_type>
size_t count_keys_with_prefix_cpp11(const map_type& map, const std::string& prefix) {
    size_t count = 0;
    for (typename map_type::const_iterator it = map.begin(); it != map.end(); ++it) {
