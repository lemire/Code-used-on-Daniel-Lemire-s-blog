...
struct bitmask {
  explicit bitmask(size_t capacity)
      : line_start((capacity + 63) / 64), hash((capacity + 63) / 64), comment((capacity + 63) / 64) {}
...
bitmask compute_bitmask(std::string_view input) {
  ...
  for (; i + 16 <= input.size(); i+=16) {
    ...
    memcpy(reinterpret_cast<uint8_t*>(answer.line_start.data()) + i / 8, &endoflinemask, sizeof(uint16_t));
    memcpy(reinterpret_cast<uint8_t*>(answer.hash.data()) + i / 8, &hashmask, sizeof(uint16_t));
  }
  for (; i < input.size(); i++) {
    if (input[i] ==
