...
bool craftedhash_is_special(std::string_view input) {
  ...
  const static std::string_view table[] = {"http", "", "https", "ws", "ftp", "wss", "file", ""};
  return input == table[hash_value];
}
...
bool craftedhash2_is_special(std::string_view input) {
  static const char* table[] = {"http", "", "https", "ws", "ftp", "wss", "file", ""};
  static size_t table_length[] = {4, 0, 5, 2, 3, 3, 4, 0};
  if(input.empty()) { return false; }
  int hash_value = (2*input.size() + (unsigned)(input[0])) & 7;
  if(table_length[hash_value] != input.size()) { return false; }
  const char* target = table[hash_value];
...
bool craftedhash2b_is_special(std::string_view input) {
  static const char* table[] = {"http", "", "https", "ws", "ftp", "wss", "file", ""};
  static size_t table_length[] = {4, 0, 5, 2, 3, 3, 4, 0};
  if(input.empty()) { return false; }
  int hash_value = (2*input.size() + (unsigned)(input[0])) & 7;
...
void simulation(size_t N) {
...

  {
    ...
    for (; finish - start < threshold;) {
      ...
      matches = 0;
      for (auto v : data) {
        matches += craftedhash2_is_special(v);
      }
      finish = nano();
    }
    double t = double(finish - start) / (N * count);

    printf("hash2 %f ns/string, matches = %zu \n", t, matches);
  }

  {
    ...
    for (; finish - start < threshold;) {
      ...
      matches = 0;
      for (auto v : data) {
        matches += craftedhash2b_is_special(v);
      }
      finish = nano();
    }
    double t = double(finish - start) / (N * count);

    printf("hash2b %f ns/string, matches = %zu \n", t, matches);
  }
}
