...
Each panel of the figure is one document being parsed, and the three bars inside it are the three PGO builds: the binary trained on `twitter.json`, the one trained on `canada.json`, and the one trained on `citm_catalog.json`. Bar height is the speed gain over the ordinary, profile-free build of that same document, in percent, so zero means PGO changed nothing and a bar below the axis means the PGO build was slower. The green bar in each panel is the matched case, where the profile was collected on the very document being measured; the grey bars show what the same profile does for the two documents it never saw. Reading down the diagonal gives the best case for PGO — +3.1%, +4.7%, +0.8% — and reading across each row shows how well a profile transfers. Every bar sits between roughly -0.5% and +5%, which is the main thing the plot conveys: for this workload the whole effect of PGO, good or bad, lives in a narrow band of a few percent.

The gains are modest. The best result is `canada.json` at +4.7%, and the typical gain is in the 2–3% range. Profiling one document usually helps the others, but not reliably. Profiling `twitter.json` gave a decent improvement everywhere: +3.1%, +2.0%, +2.8%. But profiling `canada.json` bought 4.7% on `canada.json` and essentially nothing anywhere else. Interestingly, profiling `citm_catalog.json` produced a mere +0.8% on its own document while helping `twitter.json` more.
