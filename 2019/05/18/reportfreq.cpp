...
double measure_frequency_daniel() {
...
// tries to estimate the frequency, returns false on failure
 __attribute__ ((noinline))
double measure_frequency_travis() {
  const size_t test_duration_in_cycles =
     65536;
...
void demo_daniel(int attempt) {
  std::cout << "measure using a tight loop:" << std::endl;
  std::vector<double> freqs;
  for (int i = 0; i < attempt; i++) {
    double freq = measure_frequency_daniel();
...
void demo_travis(int attempt) {
  std::cout << "measure using an unrolled loop:" << std::endl;
  std::vector<double> freqs;
  for (int i = 0; i < attempt; i++) {
    double freq = measure_frequency_travis();
    if(freq > 0) freqs.push_back(freq);
  }
  if(freqs.size() == 0) {
    std::cout << "could not collect a measure" << std::endl;
