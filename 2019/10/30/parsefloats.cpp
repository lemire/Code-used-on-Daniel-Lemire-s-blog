...
/*
// unsupported by GNU GCC !!!
double sum_fromchars(std::string mystring, size_t howmany) {
  const char * s = mystring.data();
  size_t length = mystring.size();
  double answer = 0;
  const char * lst;
...
void demo(uint64_t howmany) {
  ...
  for (size_t trial = 0; trial < 3; trial++) {
    ...
    printf("stream    %.2f cycles/value %.2f instructions/value branch "
  ...
  for (size_t trial = 0; trial < 3; trial++) {
    ...
    if(ts == 0) printf("bug\n");
    double dif = std::chrono::duration_cast<std::chrono::nanoseconds>( t2 - t1 ).count();
    printf("strtod   %.2f cycles/value %.2f instructions/value branch "
           "misses/value %.2f  MB/s: %.2f cycles/byte: %.2f \n",
           results[0] * 1.0 / howmany, results[1] * 1.0 / howmany,
           results[2] * 1.0 / howmany, volume * 1000000000/ dif, results[0]/(s.size() * 1.));
  }
...
void demoints(uint64_t howmany) {
  ...
  for (size_t trial = 0; trial < 3; trial++) {
    ...
    std::chrono::high_resolution_clock::time_point t1 = std::chrono::high_resolution_clock::now();
    unified.start();
    uint64_t ts = sumints_fromchars(s, howmany);
    unified.end(results);
    std::chrono::high_resolution_clock::time_point t2 = std::chrono::high_resolution_clock::now();
    if(ts == 0) printf("bug\n");
    double dif = std::chrono::duration_cast<std::chrono::nanoseconds>( t2 - t1 ).count();
 
    printf("fromchars    %.2f cycles/value %.2f instructions/value branch "
           "misses/value %.2f MB/s: %.2f cycles/byte: %.2f \n",
           results[0] * 1.0 / howmany, results[1] * 1.0 / howmany,
           results[2] * 1.0 / howmany, volume *1000000000/ dif, results[0]/(s.size()*1.));
  }
