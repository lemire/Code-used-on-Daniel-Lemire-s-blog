...
uint32_t get8(size_t index, uint8_t *data) {
  return data[index];
}
...
// sets a 12-bit value at index to packed 12-bit values datal, val should be a 12-bit value
...
uint32_t sum8(uint8_t *data, size_t number) {
  uint32_t s = 0xFFF;
...
__attribute__((optimize("no-tree-vectorize")))
uint32_t sum(uint8_t *data, size_t number) {
  uint32_t s = 0xFFF;
  for (size_t i = 0; i < number; i++)
    s += get(i, data);
  return s;
...
__attribute__((optimize("no-tree-vectorize")))
uint32_t sumthomas(uint8_t *data, size_t number) {
  uint32_t s = 0xFFF;
  for (size_t i = 0; i < number; i++)
    s += getthomas(i, data);
  return s;
...
__attribute__((optimize("no-tree-vectorize")))
bool populate(uint8_t *data, size_t number) {
  for (size_t i = 0; i < number; i++)
    set(i, 0xFFF - (i & 0xFFF), data);
  return true;
}
__attribute__((optimize("no-tree-vectorize")))
bool populatethomas(uint8_t *data, size_t number) {
  for (size_t i = 0; i < number; i++)
...
void demo(size_t number) {
  ...
  BEST_TIME(sumthomas(data, number), expectedthomas, , repeat, number, number, true);
