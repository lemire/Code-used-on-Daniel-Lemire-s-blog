...
void test(uint32_t bit) {
    ...
    for(size_t length = N ; length <= N; ++length) {
        for(size_t i = 0 ; i < N; ++i) {
            backdata[i] = 0;
        }
        simdpack_length(0, data, length, (__m128i *) buffer, bit);
        simdunpack_length(0, (__m128i *) buffer, length, backdata, bit);
