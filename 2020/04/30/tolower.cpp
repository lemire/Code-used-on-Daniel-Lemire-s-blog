...
bool compare_ascii(uint64_t w1, uint64_t w2) {
    ...
    // validate if w1 contains only A-Z/a-z chars (then w2 must also be in this range)
    w1 |= packed_byte(0x20);
...
// are they ASCII *and* identical up to case?
bool equal_ascii_ignore_case(uint64_t w1, uint64_t w2) {
    ...
    const uint64_t is_diff = (diff << 2);
    // check if any non-ascii
    const uint64_t any_non_ascii = (w1 | w2) & packed_byte(0x80);
    if (any_non_ascii) {
        // for safety, reject if any has non-ASCII
        return false;
    }
    w1 |= packed_byte(0x20); // if it were in A-Z, it becomes a-z, otherwise, it remains there
    const uint64_t A = w1 + packed_byte(128 - 'a');
    const uint64_t Z = w1 + packed_byte(128 - 'z' - 1);
    const uint64_t mask_lower = (A ^ Z) & packed_byte(0x80);
...
void demo() {
  ...
  for (size_t trial = 0; trial < 5; trial++) {
    ...
    std::cout << "mula ";
    t = Timer{"mula"};
    is_the_same = (is_ascii_equal_ignoring_case_mula((const char *)random.data(),
                               (const char *)randomlower.data(), N));
    if (!is_the_same) {
      std::cerr << "bug" << std::endl;
      abort();
    }
    ...
    std::cout << "mula (2) ";
    t = Timer{"mula (2) "};
    is_the_same = (is_ascii_equal_ignoring_case_mula2((const char *)random.data(),
                               (const char *)randomlower.data(), N));
    if (!is_the_same) {
      std::cerr << "bug" << std::endl;
      abort();
