...
int main() {
  for (size_t k = 5; k <=20; k++) {
    ...
    for (size_t i = 1; i < N; i++) {
        if(buffer[i] < buffer[i-1]) { printf("bug\n"); }
    }
    for (size_t i = 0; i < N; i++) {
      buffer[i] = original[i];
    }
    counter = 0;
    heapsort((void *)buffer, N, sizeof(double), compare);
    printf("heapsort comparisons %f\n ", float(counter)/N);
    ...
    counter = 0;
    mergesort((void *)buffer, N, sizeof(double), compare);
    printf("mergesort comparisons %f\n ", float(counter)/N);
    ...
    counter = 0;
    std::sort(buffer, buffer+N, comparestl);
