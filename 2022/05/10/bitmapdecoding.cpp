...
#if defined(__AVX512VL__) && defined(__AVX512VL__)
...
static inline void vbmi2_decoder_permutexvar(uint32_t *base_ptr, uint32_t &base,
                                           uint32_t idx, uint64_t bits) {
  __m512i indicies = _mm512_maskz_compress_epi8(bits, _mm512_set_epi32(
  ...
  // re-arrange for storage
  indicies = _mm512_permutexvar_epi8(_mm512_set_epi32(
  ...
  __m512i index_mask = _mm512_set1_epi32(0xff);
  __m512i start_index = _mm512_set1_epi32(idx);
  
  __m512i masked_indicies = _mm512_and_si512(indicies, index_mask);
  _mm512_storeu_si512(base_ptr + base, _mm512_add_epi32(masked_indicies, start_index));
  
  masked_indicies = _mm512_and_si512(_mm512_srli_epi32(indicies, 8), index_mask);
  _mm512_storeu_si512(base_ptr + base + 16, _mm512_add_epi32(masked_indicies, start_index));
  
  masked_indicies = _mm512_and_si512(_mm512_srli_epi32(indicies, 16), index_mask);
  _mm512_storeu_si512(base_ptr + base + 32, _mm512_add_epi32(masked_indicies, start_index));
  
  masked_indicies = _mm512_srli_epi32(indicies, 24);
  _mm512_storeu_si512(base_ptr + base + 48, _mm512_add_epi32(masked_indicies, start_index));
...
void test(const char *filename, char target) {
  ...
  double avgtime = std::accumulate(timings.begin(), timings.end(),decltype(timings)::value_type(0)) / timings.size();
  printf("time [%.3f,%.3f] ns per value set\n", mintime/matches, avgtime/matches);
...
#endif
#ifdef __AVX512VBMI2__
  unit<vbmi2_decoder_permutexvar>();
  printf("vbmi2_decoder_permutexvar:\n");
  test<vbmi2_decoder_permutexvar>("nfl.csv", ',');

  unit<vbmi2_decoder_cvtepu8>();
  printf("vbmi2_decoder_cvtepu8:\n");
  test<vbmi2_decoder_cvtepu8>("nfl.csv", ',');

  unit<vbmi2_decoder_cvtepu8_branchy>();
  printf("vbmi2_decoder_cvtepu8_branchy:\n");
  test<vbmi2_decoder_cvtepu8_branchy>("nfl.csv", ',');
