...
bool mask3(std::string_view input) {
  const char *str = input.data();
...
__attribute__((noinline))
bool no_inline_mask3(std::string_view input) {
  const char *str = input.data();
  uint8_t mask1, mask2, mask3;
...
bool mask_is_special(std::string_view input) {
  uint8_t mask = ~0;
  uint64_t inputu = string_to_uint64(input);
  const char *str = (char *)&inputu;
...
__attribute__((noinline))
bool no_inline_mask_is_special(std::string_view input) {
  uint8_t mask = ~0;
  uint64_t inputu = string_to_uint64(input);
  const char *str = (char *)&inputu;
...
bool mask2_is_special(std::string_view input) {
  uint8_t mask = ~0;
  uint64_t inputu = string_to_uint64(input);
  const char *str = (char *)&inputu;
...
__attribute__((noinline))
bool no_inline_mask2_is_special(std::string_view input) {
  uint8_t mask = ~0;
  uint64_t inputu = string_to_uint64(input);
  const char *str = (char *)&inputu;
...
void simulation(size_t N) {
...

  {
    ...
    printf("mask3 %f ns/string, matches = %zu \n", t, matches);
  }
  ...
  }
  {
    ...
    for (; finish - start < threshold;) {
      ...
      matches = 0;
      for (auto v : data) {
        matches += mask_is_special(v);
      }
      finish = nano();
    }
    double t = double(finish - start) / (N * count);

    printf("mask_is_special %f ns/string, matches = %zu \n", t, matches);
  }
  {
    uint64_t start = nano();
    uint64_t finish = start;
    ...
    for (; finish - start < threshold;) {
      ...
      matches = 0;
      for (auto v : data) {
        matches += mask2_is_special(v);
      }
      finish = nano();
    }
    double t = double(finish - start) / (N * count);

    printf("mask2_is_special %f ns/string, matches = %zu \n", t, matches);
  }
  {
    uint64_t start = nano();
    uint64_t finish = start;
...

  {
    ...
    for (; finish - start < threshold;) {
      ...
      matches = 0;
      for (auto v : data) {
        matches += no_inline_mask3(v);
      }
      finish = nano();
    }
    double t = double(finish - start) / (N * count);

    printf("no_inline_mask3 %f ns/string, matches = %zu \n", t, matches);
  }
  ...
  }
  {
    ...
    for (; finish - start < threshold;) {
      ...
      matches = 0;
      for (auto v : data) {
        matches += no_inline_mask_is_special(v);
      }
      finish = nano();
    }
    double t = double(finish - start) / (N * count);

    printf("no_inline_mask_is_special %f ns/string, matches = %zu \n", t, matches);
  }
  {
    uint64_t start = nano();
    uint64_t finish = start;
    ...
    for (; finish - start < threshold;) {
      ...
      matches = 0;
      for (auto v : data) {
        matches += no_inline_mask2_is_special(v);
      }
      finish = nano();
    }
    double t = double(finish - start) / (N * count);

    printf("no_inline_mask2_is_special %f ns/string, matches = %zu \n", t, matches);
  }
  {
    uint64_t start = nano();
    uint64_t finish = start;
