...
parse_eight_digits_unrolled(const unsigned char *chars) {
  uint32_t x = 10000000 * (chars[0]) + 1000000 * (chars[1]) +
               ...
               100 * (chars[5]) + 10 * (chars[6]) + (chars[7]);
...
parse_eight_digits_ssse3(const unsigned char *chars) {
  ...
  const __m128i t1 =
      _mm_maddubs_epi16(input, mul_1_10); // a0 + 10 * a1,  a2 + 10 * a3,  a4 +
                                          // 10 * a5,  a6 + 10 * a7,...
  const __m128i t2 =
      _mm_madd_epi16(t1, mul_1_100); // a0 + 10 * a1 + 100 * a2 + 1000 * a3,  a4
                                     // + 10 * a5 + 100*  a6 + 1000 * a7,...
