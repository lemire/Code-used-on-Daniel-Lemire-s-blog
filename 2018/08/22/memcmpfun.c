...
int hashcmp(const unsigned char *sha1, const unsigned
 char *sha2)
   {
...
void demo(size_t N) {
  ...
  BEST_TIME(mass_comparison_bcmp(bigarray1, bigarray2,comparisonsB, N), N,
            , repeat, N, N * STRINGLENGTH, verbose);
  for(size_t i = 0; i < N; i++) {
    if(comparisonsA[i] == 0) assert(comparisonsB[i]);
    if(comparisonsB[i]) assert(comparisonsA[i] == 0);
  }

  BEST_TIME(mass_comparison_hash(bigarray1, bigarray2,comparisonsB, N), N,
            , repeat, N, N * STRINGLENGTH, verbose);
  for(size_t i = 0; i < N; i++) {
    if(comparisonsA[i]>0) assert(comparisonsB[i]>0);
    if(comparisonsA[i]>0) assert(comparisonsB[i]<0);
    if(comparisonsA[i]==0) assert(comparisonsB[i]==0);
  }
  free(bigarray1);
