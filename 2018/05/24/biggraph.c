...
size_t make_connected(node_t *biggraph, size_t number_of_nodes, size_t start_node) {
  size_t arraysize =
      (number_of_nodes * 64 + sizeof(uint64_t) * 8 - 1) / (sizeof(uint64_t) * 8);
  ...
  for (size_t i = 0; i < arraysize; ++i ) {
      ...
      while (w != 0) {
        ...
        biggraph[newsize++] = biggraph[r + base];
        w ^= t;
      }
      base += 64;
  }
...
size_t lastvisited(node_t *biggraph, size_t number_of_nodes, size_t start_node, size_t *visited) {
  *visited = 0;
  size_t arraysize =
      (number_of_nodes * 64 + sizeof(uint64_t) * 8 - 1) / (sizeof(uint64_t) * 8);
  uint64_t *bitset = (uint64_t *)malloc(sizeof(uint64_t) * arraysize);
...
int64_t distance(node_t *biggraph, size_t number_of_nodes, size_t start_node,
  ...
  while (queue_size > 0) {
    ...
    for (size_t i = 0; i < buffer_size; i++) {
      node_t * queued_node = & biggraph[buffer[i]];
      if(prefetching) __builtin_prefetch(biggraph[buffer[i+1]].adjacent, 1,3);
      if (unlikely(queue_size + NODE_DEGREE > queue_capacity)) {
        queue_capacity *= 2;
        queue = (size_t *)realloc(queue, queue_capacity * sizeof(size_t));
        buffer = (size_t *)realloc(buffer, queue_capacity * sizeof(size_t));
...
void demo(size_t number_of_nodes) {
  ...
  printf("making the graph connected... starting of with %zu potential nodes.\n",number_of_nodes);
  number_of_nodes = make_connected(biggraph, number_of_nodes, start_node);
  printf("Number of nodes %zu \n", number_of_nodes);
  size_t volume = 0;
  size_t final_node = lastvisited(biggraph,  number_of_nodes,  start_node, &volume);
  // we seek the distance
  int64_t expected = distance(biggraph, number_of_nodes, start_node, final_node, false);
  printf("distance %lld \n", expected);
  int repeat = 5;
  bool USE_PREFETCH = true;
  bool DO_NOT_USE_PREFETCH = false;

  BEST_TIME(distance(biggraph, number_of_nodes, start_node, final_node, DO_NOT_USE_PREFETCH),
            expected, , repeat, volume, true);
  BEST_TIME(distance(biggraph, number_of_nodes, start_node, final_node, USE_PREFETCH),
            expected, , repeat, volume, true);
...
int main() {
  for(size_t number_of_nodes = 100000; number_of_nodes <= 100000000; number_of_nodes *= 10) {
      demo(number_of_nodes);
      printf("\n");
  }
}
